 ...
 * * Displays detailed information about a single patient appointment.
 * features:
 * - Dynamic status badging (colors based on state)
 * - Quick actions (Confirm/Cancel) with loading protection
 * - Responsive layout for mobile/desktop
 * - Dark mode support
 * * @param {string} id - Unique appointment identifier
 * @param {function} onStatusUpdate - Callback to trigger state mutation (AppSync simulation)
 * @param {boolean} isUpdating - Loading flag to prevent double-submission
 */
export function AppointmentCard({ 
  ...
  /**
   * Safe Handler
   * Prevents user from triggering multiple updates while a mutation is already in flight.
   */
  ...
  return (
    // Card Container: interactive hover states and dark mode backgrounds
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 shadow-sm hover:shadow-md transition-all duration-200">
      <div className="flex items-start justify-between">
        ...
        {/* Left Section: Patient & Appointment Details */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-4">
                ...
                <div className="mt-2 flex items-center text-sm text-gray-500 dark:text-gray-400">
                   <Video className="flex-shrink-0 mr-1.5 h-4 w-4 text-gray-400 dark:text-gray-500" />
                   {consultationType}
                </div>
              </div>
        ...
        <div className="ml-4 flex items-center space-x-2">
          ...
          {status !== 'Confirmed' && status !== 'Completed' && status !== 'Cancelled' && (
            <button 
              ...
              disabled={isUpdating} 
              className="p-2 rounded-full text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 disabled:opacity-50 transition-colors"
          ...
          {status !== 'Cancelled' && status !== 'Completed' && (
            <button 
              ...
              disabled={isUpdating} 
              className="p-2 rounded-full text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 disabled:opacity-50 transition-colors"
          ...
          {/* Context Menu (Placeholder for future features like 'Reschedule') */}
          <button 
